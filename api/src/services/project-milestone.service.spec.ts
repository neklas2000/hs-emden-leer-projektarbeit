...
describe('Service: ProjectMilestoneService', () => {
	...
	it('should find all milestones', (done) => {
		jest.spyOn(repository, 'find').mockResolvedValue(['Milestone 1', 'Milestone 2'] as any[]);
	...
	it('should create a new milestone and delete the id', (done) => {
		jest.spyOn(repository, 'create').mockReturnValue({ name: 'Milestone' } as any);
		...
		service
			...
			.then((result) => {
				...
				expect(repository.save).toHaveBeenCalledWith({ name: 'Milestone' }, { reload: true });
				expect(result).toEqual('Milestone');
