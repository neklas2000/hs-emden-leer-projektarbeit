...
export class ProjektarbeitAPI1722710983371 implements MigrationInterface {
	name = 'ProjektarbeitAPI1722710983371';

	public async up(queryRunner: QueryRunner): Promise<void> {
		await queryRunner.query(`ALTER TABLE \`project_member\` DROP COLUMN \`invite_pending\``);
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`ALTER TABLE \`project_member\` ADD \`invite_pending\` tinyint NOT NULL DEFAULT 1`,
		);
	}
}
